...
class MaxDeliveriesDlqRepublisherIT {
    ...
    fun `republishes original payload to dlq subject after max-deliveries exhausted`() {
        val durable = "grid-user-deleted-it-${UUID.randomUUID().toString().take(6)}"
        val js = connection.jetStream()
        val mgmt = connection.jetStreamManagement()

        // Always-failing consumer to force redelivery exhaustion.
        ...
        val sub = js.subscribe("wordsparrow.user.deleted", opts)
        val pumpThread =
            Thread {
                try {
