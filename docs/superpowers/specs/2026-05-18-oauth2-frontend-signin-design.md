...
4. **First-sign-in detection.** `AuthProvider` is the owner. After a successful `whoami()`, if `whoami.displayName === 'Joueur'` (the server default) AND `localStorageSession.getPseudonym()` returns a non-default value, it calls `updateMe(localStoragePseudonym)` once and refreshes its own state from the response. Idempotent and self-disabling on subsequent boots (after the patch, `displayName !== 'Joueur'`).
