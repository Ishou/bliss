// `/sondage/pairs` eager half — route definition + head(); lazy UI in `./sondage.pairs.lazy`.
...
function SondagePairsSkeleton() {
  return (
    <ContentPage>
      <article className={skeletonArticleStyles}>
        <h1 className={skeletonHeadingStyles}>Sondage par paires</h1>
...
export const Route = createRoute({
  ...
  path: '/sondage/pairs',
  pendingMs: 0,
  pendingComponent: SondagePairsSkeleton,
  head: () =>
    buildHead({
      title: 'Sondage par paires — WordSparrow',
      description:
        'Aidez à départager deux définitions générées pour le même mot.',
      canonical: `${SITE_BASE_URL}/sondage/pairs`,
      noindex: true,
    }),
}).lazy(() => import('./sondage.pairs.lazy').then((m) => m.Route));
