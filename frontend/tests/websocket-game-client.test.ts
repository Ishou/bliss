...
describe('WebSocketGameClient page-lifecycle leave', () => {
  // Real page teardown should send leaveLobby so the server can free the
  // slot immediately rather than wait out the ADR-0018 §5 30s grace.
  // bfcache hand-offs (`pagehide` with persisted=true) must NOT, otherwise
  // a back/forward navigation would kick the player.

  it('sends leaveLobby on pagehide when persisted=false', async () => {
    const { ws } = await connectAndOpen();
    const sentBefore = ws.sent.length;
    window.dispatchEvent(
      new PageTransitionEvent('pagehide', { persisted: false }),
    );
    const sentAfter = ws.sent.slice(sentBefore);
    expect(sentAfter).toHaveLength(1);
    expect(JSON.parse(sentAfter[0]!)).toEqual({ type: 'leaveLobby' });
  });

  it('does NOT send leaveLobby on pagehide when persisted=true (bfcache)', async () => {
    const { ws } = await connectAndOpen();
    const sentBefore = ws.sent.length;
    window.dispatchEvent(
  ...
  it('sends leaveLobby on beforeunload', async () => {
    const { ws } = await connectAndOpen();
    const sentBefore = ws.sent.length;
  ...
  it('detaches the lifecycle listeners on close so a stale handler does not fire', async () => {
    const { client, ws } = await connectAndOpen();
    client.disconnect();
    const sentBefore = ws.sent.length;
    // After disconnect, dispatching `pagehide` should not enqueue a new
    // frame (no socket to send through, and the listener should be gone).
    window.dispatchEvent(
      new PageTransitionEvent('pagehide', { persisted: false }),
    );
    expect(ws.sent.length).toBe(sentBefore);
