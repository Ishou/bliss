...
|---|---|---|---|
| `sessionId` (identifiant aléatoire UUID v7) | Généré par votre navigateur, stocké dans `localStorage` | Identifier votre session pour le multijoueur, compter vos demandes d'indices, et varier les libellés affichés d'une grille à la suivante | Tant que vous ne videz pas le stockage du navigateur ou ne cliquez pas sur "Effacer mes données" |
...
|---|---|---|---|
| `sessionId` (random UUID v7) | Generated by your browser, stored in `localStorage` | Identify your session for multiplayer, count your hint requests, and vary the clue text shown across consecutive grids | Until you clear browser storage or click "Erase my data" |
