...
import com.bliss.grid.application.lexicon.ImportWordsUseCase
import com.bliss.grid.infrastructure.persistence.JdbcWordCorpusWriter
...
class ImportWordsCommand : CliktCommand(name = "import-words") {
    ...
    internal fun executeImport(
        ...
        withCorrelationId {
            ...
            val writer = JdbcWordCorpusWriter(ds, batchSize)
            val report = ImportWordsUseCase(writer).execute(lines, language, source, sourceLicense)
            log.info("import_words_filter_complete total_read={} kept={}", report.totalRead, report.kept)
            log.info(
                "import_words_complete total_read={} kept={} inserted={} skipped_on_conflict={}",
                report.totalRead,
                report.kept,
                report.insert.inserted,
                report.insert.skipped,
            )
        }
    }
