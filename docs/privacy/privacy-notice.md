...
| Indices demandés par grille | Bouton "indice" pendant une partie | Limiter le nombre d'indices par grille | 90 jours après votre dernière demande, puis suppression automatique ; les sauvegardes Postgres (chiffrées) peuvent conserver ces données au-delà de cette fenêtre selon la politique de sauvegarde (ADR-0010) |
...
- **Droit à l'effacement** (article 17) : utilisez le bouton "Effacer
  ...
  pseudonyme et l'historique d'indices sont supprimés immédiatement ;
  les sauvegardes Postgres (chiffrées) peuvent conserver l'historique
  d'indices au-delà de la fenêtre de 90 jours (ADR-0010). Les visites
  Matomo du **jour en cours** sont effacées. Les visites des jours
  précédents sont déjà non-attribuables (le hachage rotatif quotidien
  empêche tout recoupement inter-journalier) ; elles ne peuvent pas être
  activement supprimées mais restent soumises à la fenêtre de
...
| Hints used per puzzle | "Hint" button during a game | Cap hints per puzzle | 90 days from your last request, then auto-deleted; encrypted Postgres backups may retain this data beyond that window per the backup policy (ADR-0010) |
...
- **Erasure** (Article 17): use the "Erase my data" button in settings.
  ...
  encrypted Postgres backups may retain hint history beyond the 90-day
  window (ADR-0010). **Today's** Matomo visits are removed. Visits from
  prior days are already non-attributable (the hash rotates daily;
  cross-day linkage is impossible by design) and cannot be actively
  deleted; they remain subject to the 13-month retention window.
